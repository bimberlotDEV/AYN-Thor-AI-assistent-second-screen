...
The direct-download APK in `downloads/` is the minified `1.1.3-game-detection` test release. It is signed with the same local Android test/debug certificate as the previous private build so compatible test installations can be updated in place; it is not a long-term production-signed release.
